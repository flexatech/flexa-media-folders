...
interface FolderNodeProps {
    ...
    cutMarked?: boolean;
    canPaste?: boolean;
    /** When false, the row is read-only: no drag handle and no context menu.
     *  Structural mutations are gated server-side by `manage_categories`; this
     *  just hides the controls users without that capability cannot use. */
    canEdit?: boolean;
    onSelect: () => void;
    onToggle: () => void;
    onCreateChild: () => void;
...
function readDragPayload(
    ...
    const raw =
        event.dataTransfer.getData(ATTACHMENT_MIME) ||
...
export const FolderNode = forwardRef<HTMLDivElement, FolderNodeProps>(
    function FolderNode(
        ...
        const row = (
            <div
                ...
                className={cn(
                    ...
                    selected &&
                        "fmf:bg-brand-50 fmf:font-medium fmf:text-brand-700",
                ...
                style={{
                    ...
                    paddingTop:
                        insertHint === "before" ? GHOST_ROW_HEIGHT : undefined,
                    paddingBottom:
                        insertHint === "after" ? GHOST_ROW_HEIGHT : undefined,
                }}
                {...draggable.attributes}
                {...(renaming || !canEdit ? undefined : draggable.listeners)}
                role="treeitem"
            ...
            >
                {Array.from({ length: depth }, (_, k) => {
                    ...
                    if (isParentCol) {
                        height = item.lastChildPath[depth] ? "half" : "full";
                    } else {
                        height = !item.lastChildPath[k + 1] ? "full" : null;
                    }
                    if (height === null) {
                        return null;
                    }
                ...
                {item.hasChildren ? (
                    <button
                        ...
                        aria-label={
                            item.collapsed ? __("Expand") : __("Collapse")
                        }
                    >
                        {item.collapsed ? (
                            <Folder
        ...
        // Read-only users can still select folders and receive attachment
        // drops (gated separately by `upload_files`), but the structural
        // context menu is hidden entirely.
        if (!canEdit) {
            return row;
        }
